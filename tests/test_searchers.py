...
def multiply_search_space_fn():
    ...
    class Multiply(Module):
        ...
        def __init__(self, compile_fn, *args, **kwargs):
            ...
            self._fn = None
            self._register(['In'], ['Out'],
                           {'multiplier': Discrete(['1', '2', '4', '8'])})
...
class Tmp(object):
    ...
    def forward(self, name_to_val):
        if self._module_seq is None:
            self._module_seq = determine_module_eval_seq(
                self.name_to_input.values())
        input_name_to_val = {
            ix: name_to_val[name] for name, ix in iteritems(self.name_to_input)
        }
        forward(input_name_to_val, self._module_seq)
        output_name_to_val = {
            name: ox.val for name, ox in iteritems(self.name_to_output)
        }
