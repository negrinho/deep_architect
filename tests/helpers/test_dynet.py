...
def evaluate(x_train, y_train, inputs, outputs):
    ...
    for (x_ins, y_ins) in zip(x_train, y_train):
        ...
        if (logits.value() >= 0.5 and
                almost_equal(y_ins, 1)) or (logits.value() < 0.5 and
                                            almost_equal(y_ins, 0)):
            correct += 1
...
def main():
    ...
    for _ in xrange(num_sample):
        ...
        val_acc = evaluate(x, y, inputs, outputs)['val_accuracy']
