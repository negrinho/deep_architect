...
def Flatten(): 
    def cfn(di, dh): 
        def fn(di): 
            return {'Out': tf.keras.layers.Flatten() (di['In'])} 
        return fn
...
def Dense(h_units): 
    def cfn(di, dh):
        def fn(di):
            return {'Out' : tf.keras.layers.Dense(dh['units']) (di['In'])}
        return fn
    ...
    def cfn(di, dh):
        def fn(di):
            ...
            if nonlin_name == 'relu':
                ...
                Out = tf.keras.layers.Activation('softplus')(di['In'])
            else:
                raise ValueError
            return {"Out" : Out}
        return fn
...
def Dropout(h_keep_prob):
    def cfn(di, dh):
        def fn(di):
            return {'Out' : tf.keras.layers.Dropout(dh['keep_prob'])(di['In'])}
        return fn
...
def BatchNormalization():
    def cfn(di, dh):
        def fn(di):
            return {'Out' : tf.keras.layers.BatchNormalization()(di['In'])}
        return fn
...
def main():
    ...
    evaluator = SimpleClassifierEvaluator((x_train, y_train), (x_test, y_test), num_classes,
                './temp', max_num_training_epochs=5, log_output_to_terminal=True) 
...
class SimpleClassifierEvaluator:
    ...
    def get_optimizer(self): 
        ...
        if self.optimizer_type == 'adam':
            ...
            optimizer = tf.keras.optimizers.SGD(lr=self.learning_rate, momentum=0.99)
        else:
