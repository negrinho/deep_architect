...
class MPICommunicator(Communicator):
    def __init__(self):
        ...
        super(MPICommunicator, self).__init__(self.comm.Get_size() - 1, self.comm.Get_rank())
        self.done = False
        if self.is_master():
            self.ready_requests = [
                self.comm.irecv(source=i+1, tag=READY_REQ) for i in range(self.num_workers)]
            self.eval_requests = [
                self.comm.irecv(source=i+1, tag=RESULTS_REQ) for i in range(self.num_workers)]
            self.next_worker = -1
    ...
    def _publish_results_to_master(self, results, evaluation_id, searcher_eval_token):
        self.comm.ssend((results, evaluation_id, searcher_eval_token), dest=0, tag=RESULTS_REQ)
