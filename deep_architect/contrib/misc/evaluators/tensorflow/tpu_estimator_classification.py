...
def input_fn(features, labels, batch_size=128, train=True):
    ...
    if train:
        def augmentation(image, label):
            image = tf.random_crop(tf.image.resize_images(image['images'], (40, 40)), (32, 32, 3))
...
class AdvanceClassifierEvaluator:
    def __init__(self, train_dataset, val_dataset, num_classes,
            ...
            init_lr=.015, lr_decay_value=.97, lr_num_epochs_per_decay=2.4,
            lr_warmup_epochs=3.0, weight_decay=.001, display_step=1,
    ...
    def get_learning_rate(self, step):
        lr = tf.train.exponential_decay(
            self.init_lr, step, self.steps_per_epoch * self.lr_num_epochs_per_decay, self.lr_decay_value, staircase=True)
        warmup_steps = int(self.lr_warmup_epochs * self.steps_per_epoch)
        warmup_lr = (
            self.init_lr * tf.cast(step, tf.float32) / tf.cast(
    ...
    def eval(self, inputs, outputs):
        ...
        def model_fn(features, labels, mode, params):
            ...
            with tf.variable_scope('l2'):
                l2_loss = tf.add_n([ tf.nn.l2_loss(v) for v in train_vars if 'kernel' in v.name]) * self.weight_decay
