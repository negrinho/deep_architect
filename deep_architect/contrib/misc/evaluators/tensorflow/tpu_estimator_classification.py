...
def input_fn(mode, data_dir, batch_size=128, train=True):
    return Cifar10DataSet(
        data_dir, subset=mode, use_distortion=train).make_batch(batch_size)
            ...
            with tf.contrib.summary.always_record_summaries():
                for i, metric in enumerate(metric_names):
                    tf.contrib.summary.scalar(
                        prefix + metric, args[i][0], step=step)
...
class AdvanceClassifierEvaluator:
    ...
    def get_learning_rate(self, step):
        ...
        total_steps = int(
            self.max_num_training_epochs * self.num_examples / self.batch_size)
        if self.lr_decay_method == 'cosine_by_step':
            progress_fraction = tf.cast(step, tf.float32) / total_steps
            lr = (0.5 * self.init_lr * (1 + tf.cos(np.pi * progress_fraction)))
            ...
            lr = tf.train.exponential_decay(
                self.init_lr,
                step,
                self.steps_per_epoch * self.lr_num_epochs_per_decay,
                self.lr_decay_value,
                staircase=True)
            warmup_steps = int(self.lr_warmup_epochs * self.steps_per_epoch)
            warmup_lr = (self.init_lr * tf.cast(step, tf.float32) / tf.cast(
    ...
    def eval(self, inputs, outputs):
        ...
        def model_fn(features, labels, mode, params):
            ...
            unreg_loss = tf.losses.softmax_cross_entropy(
                onehot_labels=tf.one_hot(labels, 10), logits=logits)
            # tf.reduce_sum(
            ...
            optimizer = tf.train.RMSPropOptimizer(
                learning_rate, momentum=.9, epsilon=1.0)
            host_fn = None
            # optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
            if self.use_tpu:
                host_fn = construct_host_fn(
                    metric_dict,
                    model_dir,
                    prefix='training/',
            ...
            return tf.contrib.tpu.TPUEstimatorSpec(
                mode, loss=loss, train_op=train_op, host_call=host_fn)
        ...
        cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver(
            self.tpu_name, zone=my_zone, project=my_project_name)
        ...
        try:
            ...
            #         'epoch_number': (epoch + 1) * self.max_num_training_epochs,
            #         'time_in_minutes':
            ...
            test_results = estimator.evaluate(
                input_fn=test_fn, steps=self.steps_per_test_epoch, name='test')
