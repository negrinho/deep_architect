...
def pool_and_logits(num_classes):
    ...
    def compile_fn(di, dh):
        ...
        def fn(di):
            logits = tf.squeeze(
                tf.layers.conv2d(di['In'], num_classes, (height, width)))
            return {'Out': logits}
...
def global_pool_and_logits():
    ...
    def compile_fn(di, dh):
        ...
        def fn(di):
            logits = tf.layers.average_pooling2d(di['In'], (height, width),
                                                 (1, 1))
            return {'Out': logits}
...
def wrap_relu_batch_norm(io_pair, add_relu=True, add_bn=True):
    ...
    module_fns = [lambda: io_pair, relu, batch_normalization]
...
def wrap_batch_norm_relu(io_pair, add_relu=True, add_bn=True):
    ...
    module_fns = [lambda: io_pair, batch_normalization, relu]
    return mo.siso_sequential(
        [module_fn() for i, module_fn in enumerate(module_fns) if elements[i]])
