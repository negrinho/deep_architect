...
def conv2d(h_num_filters, h_filter_size, h_stride, h_use_bias):
    ...
    return siso_pytorch_module('Conv2D', compile_fn, {
...
def dropout(h_keep_prob):
    ...
    return siso_pytorch_module('Dropout', compile_fn, {'keep_prob' : h_keep_prob})
...
def fc_layer(h_num_units):
    def compile_fn(di, dh):
        ...
        def fn(di):
            return {'Out' : fc(di['In'])}
        return fn, [fc]
    return siso_pytorch_module('FCLayer', compile_fn, {'num_units' : h_num_units})
