...
class HyperBandExtended:
    ...
    def evaluate(self, R, eta=3):
        ...
        for s in xrange(s_max, -1, -1):
            n = int(math.ceil(
                float(B) * (eta**s) / (float(R) * (s + 1))))  # num configs
            ...
            (best_config, best_perf) = self.get_best_performance(
                T, P, best_config, best_perf)
...
class SimpleArchitectureSearchHyperBand(HyperBandExtended):
    ...
    def top_k(self, T, P, k):
        ...
        sorted_tp = sorted(
            list(zip(T, P)), key=lambda tp: tp[1][0], reverse=reverse)
