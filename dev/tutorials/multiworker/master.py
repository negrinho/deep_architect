...
def done(job_id):
    ...
    return (
        len(out.split()) == 1
    )  # another way is if checkout is ['STATE', 'PENDING']. But also risk not finishing job


def valid_filepath(result_fp):
    return False if (not os.path.isfile(result_fp) or
                     os.path.getsize(result_fp) == 0) else True
...
def main():
    ...
    for i in architectures:
        ...
        (job_id, result_fp) = create_and_submit_job(int(i), exp_name, config_fp,
                                                    evaluation_fp)
